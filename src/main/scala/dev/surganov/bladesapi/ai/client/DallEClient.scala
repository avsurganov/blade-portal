...
class DallEClient(implicit ec: ExecutionContext, mat: Materializer, sys: ActorSystem[Any]) extends ConfigProvider with LoggerAccess {
  private def optimizePrompt(description: String): String = {
    s"Create a detailed portrait of a single character from the game Blades in the Dark. Description: $description. The background should be the gloomy city of Duskvol. Ensure it is a single, focused portrait with no other characters or multiple views. No text or additional elements."
  }
